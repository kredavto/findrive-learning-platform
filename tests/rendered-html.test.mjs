...
test("contains the FINDRIVE Academy learning experience", async () => {
  ...
  assert.match(page, /Если у инвестора нет своего юридического лица, сориентируй его на заключение договора с учредителем МКК как с физлицом/i);
  assert.match(page, /Инвестор может не иметь своего юридического лица, в этом случае мы не прекращаем с ним работать/i);
  assert.match(page, /ограничивает круг инвесторов для микрофинансовых компаний только юридическими лицами/i);
  assert.doesNotMatch(page, /Не смешивай договор финансирования бизнеса с банковским вкладом или займом для клиента/i);
